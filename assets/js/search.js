{
	...
	const ImplementSongsQueue = async () => {
		try {
			songs = document.querySelectorAll(".playable-tracks");
	...
	try {
		const InitializeSongsQueue = async () => {
			onPageRefresh();
			getQueueData();
			playbar();
		};
		InitializeSongsQueue();
	} catch (error) {
		console.log(error);
	}
